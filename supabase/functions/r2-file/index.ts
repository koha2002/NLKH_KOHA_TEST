import{S3Client,PutObjectCommand,GetObjectCommand,DeleteObjectCommand}from"npm:@aws-sdk/client-s3@3";import{getSignedUrl}from"npm:@aws-sdk/s3-request-presigner@3";import{adminClient,caller,hasPermission}from"../_shared/auth.ts";import{json,corsHeaders}from"../_shared/cors.ts";
...
}const temp=`pending/${crypto.randomUUID()}`;const{data:asset,error}=await admin.from("media_assets").insert({object_key:temp,original_name:original,mime_type:mime,size_bytes:size,title:original,folder,visibility,owner_id:ctx.user?.id||null,sha256:sha,status:"pending",usage_note:String(body.usage_note||usageType),uploaded_from:isOwnAvatar?"profile":isData?"data":"admin"}).select("*").single();if(error)throw error;const objectKey=`${folder}/${code(asset.asset_no)}/${safe(original)}`,pub=visibility==="public"?publicUrl(objectKey):null;const{error:upErr}=await admin.from("media_assets").update({object_key:objectKey,public_url:pub}).eq("id",asset.id);if(upErr)throw upErr;const url=await getSignedUrl(s3,new PutObjectCommand({Bucket:b,Key:objectKey,ContentType:mime}),{expiresIn:900});return json(req,{duplicate:false,url,asset:{...asset,object_key:objectKey,public_url:pub,asset_code:code(asset.asset_no)}})}
if(action==="complete-upload"){const{data:asset}=await admin.from("media_assets").select("*").eq("id",body.media_id).maybeSingle();if(!asset)return json(req,{error:"Không tìm thấy ID R2 đang upload."},404);const own=(asset.uploaded_from==="profile"||asset.uploaded_from==="data")&&ctx.user&&asset.owner_id===ctx.user.id;if(!own&&!hasPermission(ctx,"media.manage"))return json(req,{error:"Bạn không có quyền hoàn tất upload này."},403);let completedPublicUrl=asset.public_url;
...
const{data:ready,error}=await admin.from("media_assets")
  .update({status:"ready",...(completedPublicUrl?{public_url:completedPublicUrl}:{})})
  .eq("id",asset.id)
  .select("*")
  .single();
...
if(asset.uploaded_from==="profile"){
  await admin.from("profiles")
    .update({avatar_media_id:ready.id,avatar_url:ready.public_url||completedPublicUrl||null})
    .eq("id",ctx.user.id);
}
...
if(action==="presign-download"){const key=String(body.object_key||"").replace(/^\/+/,"");let allowed=false,asset:any=null;if(body.item_id){const{data:item}=await admin.from("data_items").select("*,media_assets(*)").eq("id",body.item_id).maybeSingle();asset=item?.media_assets;if(item?.visible&&item.visibility==="public")allowed=true;else if(item?.visible&&item.visibility==="authenticated"&&ctx.user)allowed=true;else if(item?.visible&&ctx.user){if(item.collection_id)allowed=!!(await collectionPermission(admin,ctx.user.id,item.collection_id));if(!allowed){const{data:a}=await admin.from("user_data_access").select("permission_level,expires_at").eq("item_id",item.id).eq("user_id",ctx.user.id).maybeSingle();allowed=!!a&&(!a.expires_at||new Date(a.expires_at)>new Date())}}}if(!allowed&&body.media_id){const r=await admin.from("media_assets").select("*").eq("id",body.media_id).maybeSingle();asset=r.data;if(asset?.object_key!==key&&key)return json(req,{error:"ID R2 và object key không khớp."},400);if(asset?.visibility==="public")allowed=true;else if(asset?.visibility==="authenticated"&&ctx.user)allowed=true;else if(ctx.user&&(asset?.owner_id===ctx.user.id||hasPermission(ctx,"media.manage")||hasPermission(ctx,"data.manage")))allowed=true}if(!allowed)return json(req,{error:"Bạn không có quyền đọc tệp này."},403);const objectKey=asset?.object_key||key;if(!objectKey)return json(req,{error:"Không tìm thấy tệp R2."},404);const url=await getSignedUrl(s3,new GetObjectCommand({Bucket:b,Key:objectKey}),{expiresIn:900});return json(req,{url})}
