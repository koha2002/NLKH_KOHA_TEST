(function () {
  ...
  function findMainPdfInput() {
    ...
    return inputs.find(function (node) {
      if (node.id === 'watermarkImageInput') return false;
      var accept = String(node.getAttribute('accept') || '').toLowerCase();
      return accept.indexOf('pdf') !== -1;
    }) || inputs.find(function (node) { return node.id !== 'watermarkImageInput'; }) || null;
  }

  function syncPdfUiAfterClear() {
    ['fileList','selectedFilesList','selectedFileList','uploadedFiles','filesList'].forEach(function (id) {
      var node = el(id);
      if (node) node.innerHTML = '';
    });
    ['fileName','selectedFileName'].forEach(function (id) {
      var node = el(id);
      if (node) node.textContent = '';
    });
    var result = el('resultContainer');
    if (result) result.classList.add('hidden');
    hotfixOfflineResult = null;
    setBusy(false);

    // Restore the empty-state sentence without depending on a specific id.
    Array.prototype.slice.call(document.querySelectorAll('div,span,p')).forEach(function (node) {
      if (node.children.length) return;
      var text = String(node.textContent || '').trim();
      if (/^chưa có tệp nào được chọn$/i.test(text) || /^no files? selected$/i.test(text)) {
        node.style.display = '';
      }
    });
  }

  function clearMainPdfFile() {
    var input = findMainPdfInput();
    if (input) input.value = '';
    try { window.selectedFiles = []; } catch (_) {}
    try { selectedFiles = []; } catch (_) {}
    syncPdfUiAfterClear();
    updatePdfFileActions();
    if (typeof setStatus === 'function') setStatus('Đã xóa PDF đã chọn. Bạn có thể chọn tệp mới. / Selected PDF removed. You can choose another file.', 'info');
  }
  ...
  function updatePdfFileActions() {
    var wrap = el('pdfFileActionsHotfix');
    if (!wrap) return;
    var hasFile = !!(window.selectedFiles && window.selectedFiles.length);
    var input = findMainPdfInput();
    if (!hasFile && input && input.files && input.files.length) hasFile = true;
    wrap.style.display = hasFile ? 'flex' : 'none';
  }

  function installPdfFileActions() {
    var input = findMainPdfInput();
    if (!input || el('pdfFileActionsHotfix')) return;

    var wrap = document.createElement('div');
    wrap.id = 'pdfFileActionsHotfix';
    wrap.style.cssText = 'display:none;gap:8px;flex-wrap:wrap;margin-top:10px;align-items:center';
    wrap.innerHTML = '' +
      '<button type="button" id="pdfChangeFileHotfix" style="min-height:36px;padding:8px 12px;border-radius:8px;border:1px solid var(--pdf-line,#dce2ea);background:var(--pdf-surface-soft,#f7f8fa);color:inherit;font-weight:700;cursor:pointer">Thay PDF / Change PDF</button>' +
      '<button type="button" id="pdfRemoveFileHotfix" style="min-height:36px;padding:8px 12px;border-radius:8px;border:1px solid var(--pdf-line,#dce2ea);background:var(--pdf-surface-soft,#f7f8fa);color:#dc2626;font-weight:700;cursor:pointer">Xóa PDF / Remove PDF</button>';
    ...
    el('pdfChangeFileHotfix').addEventListener('click', chooseAnotherPdf);
    el('pdfRemoveFileHotfix').addEventListener('click', clearMainPdfFile);
    input.addEventListener('change', function () { setTimeout(updatePdfFileActions, 0); });

    // Some original handlers update selectedFiles asynchronously.
    var observer = new MutationObserver(function () { updatePdfFileActions(); });
    var area = host.parentElement || document.body;
