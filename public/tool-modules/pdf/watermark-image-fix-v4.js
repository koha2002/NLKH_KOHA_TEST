(function () {
  ...
  function injectStyles(){
    ...
    style.textContent=[
      ...
      '.wm-image-actions button,.wm-image-actions label,.pdf-v4-btn{min-height:36px;padding:8px 12px;border-radius:8px;border:1px solid var(--pdf-line,#dce2ea);background:var(--pdf-surface-soft,#f7f8fa);color:var(--pdf-ink,#eaf2ff);font-size:11px;font-weight:700;cursor:pointer}',
      '.wm-image-actions .danger,.pdf-v4-danger{color:#ff3b3b!important}',
      '.wm-image-controls{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}',
      '.wm-coordinate-box{grid-column:1/-1;display:none;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px;padding:10px;border:1px dashed var(--pdf-line,#304562);border-radius:8px}',
      '.wm-coordinate-box.show{display:grid}',
      ...
      '.wm-preview-head select,.wm-preview-head input,.wm-coordinate-box select,.wm-coordinate-box input{min-height:34px;border:1px solid var(--pdf-line,#304562);border-radius:7px;background:var(--pdf-surface,#0f1b2d);color:inherit;padding:6px 8px;width:100%}',
      ...
      '.pdf-v4-files{display:grid;gap:7px;margin-top:10px}',
      '.pdf-v4-file{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:8px;align-items:center;border:1px solid var(--pdf-line,#304562);border-radius:8px;padding:8px 10px;background:var(--pdf-surface-soft,#102039)}',
      '.pdf-v4-file strong{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:11px}',
      '.pdf-v4-file small{font-size:9px;color:var(--pdf-muted,#8fa2bb)}',
      '@media(max-width:700px){.wm-image-drop{grid-template-columns:1fr}.wm-image-thumb{width:100%;height:140px}.wm-image-controls{grid-template-columns:1fr 1fr}.wm-coordinate-box{grid-template-columns:1fr 1fr}.wm-preview-frame{height:420px}}'
    ].join('');
  ...
  function watermarkOptionsHtml(){
    return ''+
      ...
      '<div class="wm-image-controls">'+
        ...
        '<div class="wm-coordinate-box" id="wmCoordinateBox">'+
          '<div><label class="block text-sm font-medium">Đơn vị / Unit</label><select id="watermarkCoordinateUnit"><option value="percent">% trang / % of page</option><option value="pt">Point (pt)</option></select></div>'+
          '<div><label class="block text-sm font-medium">X — từ mép trái / from left</label><input type="number" id="watermarkX" value="10" step="0.1"></div>'+
          '<div><label class="block text-sm font-medium">Y — từ mép dưới / from bottom</label><input type="number" id="watermarkY" value="10" step="0.1"></div>'+
          '<span class="wm-note" style="grid-column:1/-1">Tọa độ đặt góc dưới-trái của ảnh dấu. Với %, X=0/Y=0 là góc dưới-trái trang; X=50/Y=50 là giữa trang theo kích thước trang. / Coordinates place the stamp image bottom-left corner.</span>'+
        '</div>'+
      '</div>'+
      '<div class="wm-preview-box" id="wmLivePreview">'+
        '<div class="wm-preview-head"><strong>Xem trước thực tế / Live preview</strong><select id="wmPreviewFile"></select><span>Trang / Page</span><button type="button" class="pdf-v4-btn" id="wmPrevPage">‹</button><input id="wmPreviewPage" type="number" min="1" value="1" style="width:68px"><button type="button" class="pdf-v4-btn" id="wmNextPage">›</button><span id="wmPreviewStatus" class="wm-note">Chọn PDF để xem trước.</span></div>'+
  ...
  function toggleCoordinateBox(){
    var box=el('wmCoordinateBox');
    if(box)box.classList.toggle('show',el('watermarkPosition') && el('watermarkPosition').value==='custom-xy');
  }
  ...
  function bindWatermarkUi(){
    ...
    ['watermarkPosition','watermarkTransparency','watermarkRotation','watermarkImageScale','watermarkCoordinateUnit','watermarkX','watermarkY'].forEach(function(id){
      var n=el(id); if(n)n.addEventListener('input',function(){toggleCoordinateBox();schedulePreview()});
      if(n)n.addEventListener('change',function(){toggleCoordinateBox();schedulePreview()});
    });
  ...
  function needsWatermarkRepair(){
    if(!isPdfWatermark())return false;
    var options=el('toolOptions');
    if(!options)return false;
    return !el('watermarkImageInput') || !!el('watermarkText') || !!el('watermarkType') || !!options.querySelector('input[type="color"]');
  }
  function scheduleRepair(){
    clearTimeout(repairTimer);
    repairTimer=setTimeout(function(){
      if(needsWatermarkRepair())applyWatermarkUi(true);
      else if(isPdfWatermark()){syncFileManager();toggleCoordinateBox();}
    },35);
  }
  function applyWatermarkUi(force){
    if(!isPdfWatermark())return;
    var options=el('toolOptions'); if(!options)return;
    if(!force && !needsWatermarkRepair() && options.dataset.wmImageOnlyV4==='1'){bindWatermarkUi();syncFileManager();schedulePreview();return}
    var existingFile=window.watermarkFile||null;
    options.innerHTML=watermarkOptionsHtml();
    options.dataset.wmImageOnlyV4='1';
    options.classList.remove('hidden');
    bindWatermarkUi();
    if(existingFile)setWatermarkFile(existingFile);else clearWatermarkFile();
    syncFileManager();
  }
  ...
  function getStampXY(pageW,pageH,drawW,drawH,position){
    ...
    if(position==='custom-xy'){
      var unit=(el('watermarkCoordinateUnit')&&el('watermarkCoordinateUnit').value)||'percent';
      var xv=Number(el('watermarkX')&&el('watermarkX').value||0);
      var yv=Number(el('watermarkY')&&el('watermarkY').value||0);
      var x=unit==='percent'?pageW*xv/100:xv;
      var y=unit==='percent'?pageH*yv/100:yv;
      return{x:Math.max(0,Math.min(pageW-drawW,x)),y:Math.max(0,Math.min(pageH-drawH,y))};
    }
    switch(position){
  ...
  function ensureFileManager(){
    ...
    if(!wrap){
      ...
      wrap.innerHTML='<button type="button" id="pdfAddFilesHotfix" class="pdf-v4-btn">Thêm PDF / Add PDF</button><div id="pdfPerFileActions" class="pdf-v4-files"></div>';
  ...
  function removePdfAt(index){
    ...
    files.splice(index,1); setSelected([]);
    var input=findMainPdfInput();
    if(input){
      try{
    ...
    hotfixResults=[]; if(el('resultContainer'))el('resultContainer').classList.add('hidden');
  ...
  function syncFileManager(){
    ...
    var files=selected(),sig=files.map(function(f){return f.name+':'+f.size+':'+f.lastModified}).join('|');
    if(sig===lastFilesSignature&&box.children.length===files.length)return;
    lastFilesSignature=sig;box.innerHTML='';
    files.forEach(function(f,i){
      var row=document.createElement('div');row.className='pdf-v4-file';
      row.innerHTML='<div><strong></strong><small></small></div><button type="button" class="pdf-v4-btn pdf-v4-danger">Xóa / Remove</button>';
  ...
  async function stampFullFile(file){
    ...
    var pages=doc.getPages(); for(var i=0;i<pages.length;i++)await drawStamp(doc,pages[i]);
  ...
  window.addEventListener('load',function(){
    ensureFileManager();applyWatermarkUi(false);syncFileManager();

    var select=el('apiTool');
    if(select)select.addEventListener('change',function(){setTimeout(function(){scheduleRepair();syncFileManager()},0)});
    document.querySelectorAll('[data-tool]').forEach(function(b){b.addEventListener('click',function(){setTimeout(scheduleRepair,0)})});

    // Original module.js rebuilds #toolOptions when switching tools.
    // Observe it and restore image-only watermark UI after the original renderer finishes.
    var target=el('toolOptions') || document.body;
    var mo=new MutationObserver(function(){ if(isPdfWatermark())scheduleRepair(); });
    mo.observe(target,{childList:true,subtree:true,attributes:false});

    setInterval(function(){
      if(isPdfWatermark())scheduleRepair();
      var sig=selected().map(function(f){return f.name+':'+f.size+':'+f.lastModified}).join('|');
      if(sig!==lastFilesSignature)syncFileManager();
    },700);
