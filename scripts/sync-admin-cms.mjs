...
try {
  const [settingsRows, nav, social, tools, toolCategories, rings, cvProfiles, cvSections, software, softwareCategories, news, newsCategories, pages, contentRoutes, seo, redirects] = await Promise.all([
    ...
    rest("tool_categories", "select=*&visible=eq.true&order=sort_order.asc"),
    ...
    rest("software_items", "select=*&visible=eq.true&order=featured.desc,sort_order.asc"),
    rest("software_categories", "select=*&visible=eq.true&order=sort_order.asc"),
  ...
  const normalizedTools = tools.map(t=>({
    ...
    requiresAuth:!!t.requires_auth, allowedRoles:t.allowed_roles||[], categoryId:t.category_id||null,
  ...
  const normalizedSoftware = software.map(x=>({
    ...
    description_vi:x.description_vi||"",
    description_en:x.description_en||x.description_vi||"",
    icon_url:x.icon_url||"",
    cover_url:x.cover_url||"",
    price_label_vi:x.price_label_vi||"Miễn phí",
    price_label_en:x.price_label_en||x.price_label_vi||"Free",
    download_access:x.download_access||"public",
    download_source:x.download_source||"link",
    download_allowed_roles:Array.isArray(x.download_allowed_roles)?x.download_allowed_roles:[],
    featured:!!x.featured,
  ...
  const normalizedNewsList = news.map(x=>({
    ...
    category_id:x.category_id||null,
    title_vi:x.title_vi||"",
    title_en:x.title_en||x.title_vi||"",
...
`export const adminToolCategories = ${ts(toolCategories)};\n`+
`export const adminOrbitRings = ${ts(normalizedRings)};\n`+
`export const adminSoftwareItems = ${ts(normalizedSoftware)};\n`+
`export const adminSoftwareCategories = ${ts(softwareCategories)};\n`+
`export const adminNewsList = ${ts(normalizedNewsList)};\n`+
